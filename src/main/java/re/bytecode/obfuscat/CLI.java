...
public class CLI {
	...
	public static void main(String[] args) {
		...
		Map<String, Object> argumentMap = parseCommandLine(args);
	...
	private static Map<String, Object> parseCommandLine(String[] args) {
		...
		if (args.length >= 1)
			argumentMap.put("command", args[0]);
		if (args.length >= 2)
			argumentMap.put("target", args[1]);

		String tag = null;
		List<String> tagData = null;
	...
	private static Map<String, Object> parsedArgs(Map<String, Object> rawArgs, Map<String, Class<?>> supportedArgs) {
		...
		for (String key : rawArgs.keySet()) {
			...
			List<String> data = (List<String>) rawArgs.get(key);

			// 0 data either means toggle
			if (data.size() == 0) {
				if (type.equals(Boolean.class)) { // no argument = True for toggles
					actualArgs.put(key, Boolean.valueOf(true));
					continue;
				} else {
					System.out.println("Builder expected argument of type " + type);
					return null;
				...
				if (data.size() == 1 && !comp.equals(String.class)
					...
					if (comp.equals(Short.class) || comp.equals(short.class)) {
						...
						System.out.println("Argument " + key + " is compatible with String casting for type " + type);
						return null;
					}
				...
				if (comp.equals(int.class)) {
					...
					actualArgs.put(key, dataArray.toArray());
				}
				continue;
			}
		}
	...
	private static boolean writeOutput(Map<String, Object> args, Object data, String defaultName) {
		if (args.containsKey("output")) {
			@SuppressWarnings("unchecked")
			List<String> outputList = (List<String>) args.get("output");
	...
	private static Object readInput(Map<String, Object> args, String defaultName) {
		if (args.containsKey("output")) {
			@SuppressWarnings("unchecked")
			List<String> outputList = (List<String>) args.get("output");
			if (outputList.size() != 1) {
	...
	private static long getSeed(Map<String, Object> argsRaw) {
		long seed = System.currentTimeMillis();
		if (argsRaw.containsKey("seed")) {
			@SuppressWarnings("unchecked")
			List<String> data = (List<String>) argsRaw.get("seed");
			if(data.size() != 1) {
	...
	private static void commandBuild(Map<String, Object> argsRaw) {

		if (!argsRaw.containsKey("target")) {
		...
		try {
			builder = Obfuscat.getBuilder((String) argsRaw.get("target"), seed);
	...
	private static void commandObfuscate(Map<String, Object> argsRaw) {
		...
		try {
			pass = Obfuscat.getPass((String) argsRaw.get("target"), seed);
		} catch (IllegalArgumentException iae) {
			System.out.println(iae.getMessage());
			return;
		}
	...
	private static void commandCompile(Map<String, Object> argsRaw) {
		...
		try {
			generator = Obfuscat.getGenerator((String) argsRaw.get("target"), f, seed);
		} catch (IllegalArgumentException iae) {
			System.out.println(iae.getMessage());
			return;
		}
